# samsampleX: An Interval Tree Based Non-Uniform Next Generation Sequencing Data Downsampling Toolkit

![Tests](https://github.com/sdemiriz/samsampleX/actions/workflows/tests.yml/badge.svg)
...
Downsample a BAM file according to a BED-defined read depth distribution, ideally a BED file generated by `map`.
```bash
samsampleX sample [options]

# Required 
--in-bam BAM              : BAM file to downsample.
...
--out-bam BAM     : Downsampled BAM file.
...
The `benchmarks/` directory contains benchmark scripts that compares `samsampleX` against uniform downsampling tools in a single- or multi-interval scenarios.
...
Generates the underlying plots for Figure 1 for comparing `samtools` uniform downsampling against `samsampleX` non-uniform downsampling.
