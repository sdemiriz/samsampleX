...
class hlaSampler(FileHandler):
    ...
    def run_sampling(
        ...
        logger.info(f"HLA-Sampler - Begin sampling")
    ...
    def get_interval_seeds(self, main_seed: int) -> np.ndarray:
        ...
        """
        logger.info(f"HLA-Sampler - Generate random seeds")
    ...
    def normalize_contig(self, contig: str) -> str:
        ...
        """
        logger.info(f"HLA-Sampler - Normalize contig name {contig}")

        if contig in self.bam.bam.references:
            logger.info(f"HLA-Sampler - Contig {contig} is as expected")
            return contig
        ...
        if normalized_contig not in self.bam.bam.references:
            raise ValueError(f"HLA-Sampler - Cannot auto-correct contig name {contig}")
    ...
    def sort_and_index(self) -> None:
        ...
        """
        logger.info(f"HLA-Sampler - Sort and index output BAM file")
